...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if config:
                self.stdout.write('SUCCESS: PayPal SDK configured successfully')
            else:
                self.stdout.write('ERROR: PayPal SDK configuration failed')
                
        except Exception as e:
            self.stdout.write(f'ERROR: PayPal SDK configuration error: {e}')
        ...
        if client_id_ok and client_secret_ok:
            self.stdout.write('SUCCESS: PayPal credentials are configured')
        else:
            self.stdout.write('WARNING: PayPal credentials need to be configured')
            if not client_id_ok:
        ...
        if client_id_ok and client_secret_ok:
            ...
            try:
                ...
                if test_payout:
                    self.stdout.write('SUCCESS: PayPal SDK can create payout objects')
                    self.stdout.write('SUCCESS: System is ready for PayPal withdrawals')
                else:
                    self.stdout.write('ERROR: PayPal SDK payout creation failed')
                    
            except Exception as e:
                self.stdout.write(f'ERROR: PayPal API connection test failed: {e}')
        ...
        try:
            ...
            if hasattr(WithdrawalService, 'process_instant_withdrawal'):
                self.stdout.write('SUCCESS: process_instant_withdrawal method exists')
            else:
                self.stdout.write('ERROR: process_instant_withdrawal method missing')
                
            if hasattr(WithdrawalService, '_process_paypal_instant'):
                self.stdout.write('SUCCESS: _process_paypal_instant method exists')
            else:
                self.stdout.write('ERROR: _process_paypal_instant method missing')
                
            if hasattr(WithdrawalService, 'queue_weekly_withdrawal'):
                self.stdout.write('SUCCESS: queue_weekly_withdrawal method exists')
            else:
                self.stdout.write('ERROR: queue_weekly_withdrawal method missing')
                
        except ImportError as e:
            self.stdout.write(f'ERROR: Cannot import WithdrawalService: {e}')
        ...
        if client_id_ok and client_secret_ok:
            ...
            self.stdout.write('WARNING: SYSTEM NEEDS CONFIGURATION')
