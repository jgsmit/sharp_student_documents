...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Check if admin can access withdrawal management
        self.stdout.write('\n--- ADMIN ACCESS TEST ---')
        try:
            ...
            # Test admin withdrawal management page
            response = client.get('/admin/manage-withdrawals/')
            if response.status_code == 200:
                self.stdout.write('Admin can access withdrawal management page')
            else:
                self.stdout.write(f'Admin access failed: {response.status_code}')
                
        except Exception as e:
            self.stdout.write(f'Error testing admin access: {e}')
        ...
        self.stdout.write('Frontend approval buttons need backend endpoints')
        self.stdout.write('JavaScript updates UI but does not process withdrawals')
