...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for field in order_fields:
                if hasattr(Order, field):
                    self.stdout.write(f'SUCCESS: Order.{field}: EXISTS')
                else:
                    self.stdout.write(f'ERROR: Order.{field}: MISSING')
            
            # Check Payment model
            payment_fields = ['order', 'payment_method', 'transaction_id', 'amount', 'status']
            for field in payment_fields:
                if hasattr(Payment, field):
                    self.stdout.write(f'SUCCESS: Payment.{field}: EXISTS')
                else:
                    self.stdout.write(f'ERROR: Payment.{field}: MISSING')
        ...
        try:
            ...
            self.stdout.write('SUCCESS: paypal_checkout: EXISTS')
            self.stdout.write('SUCCESS: paypal_return: EXISTS')
            self.stdout.write('SUCCESS: paypal_webhook: EXISTS')
        ...
        try:
            ...
            for pattern in url_patterns:
                if any(pattern in str(url.pattern) for url in urlpatterns):
                    self.stdout.write(f'SUCCESS: {pattern}: URL EXISTS')
                else:
                    self.stdout.write(f'ERROR: {pattern}: URL MISSING')
        except Exception as e:
        ...
        try:
            ...
            self.stdout.write('SUCCESS: WithdrawalRequest model: EXISTS')
            self.stdout.write('SUCCESS: WithdrawalMethod model: EXISTS')
            ...
            if hasattr(WithdrawalService, 'process_instant_withdrawal'):
                self.stdout.write('SUCCESS: WithdrawalService.process_instant_withdrawal: EXISTS')
            if hasattr(WithdrawalService, '_process_paypal_instant'):
                self.stdout.write('SUCCESS: WithdrawalService._process_paypal_instant: EXISTS')
        ...
        self.stdout.write('\n=== READINESS STATUS ===')
        if client_id_ok and client_secret_ok:
            self.stdout.write('SUCCESS: SYSTEM IS READY FOR COMPLETE PAYPAL PAYMENTS')
            self.stdout.write('SUCCESS: Buyers can pay with PayPal')
            self.stdout.write('SUCCESS: Sellers can receive PayPal payouts')
            self.stdout.write('SUCCESS: Complete payment cycle implemented')
        else:
            self.stdout.write('WARNING: PAYPAL CREDENTIALS NEEDED')
            self.stdout.write('WARNING: Configure PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET')
